...
describe('EventNotificationsV1', () => {
  ...
  test('updateSandboxDestination request example', async () => {
    consoleLogMock.mockImplementation((output) => {
      originalLog(output);
    });
    consoleWarnMock.mockImplementation((output) => {
    ...
    originalLog('updateSandboxDestination() result:');
    ...
    try {
      const res = await eventNotificationsService.updateSandboxDestination(
  ...
  test('deleteDestination request example', async () => {
    ...
    const destinations = [
      ...
      destinationId20,
      destinationId22,
      sandboxDestinationId, // Note: After updateSandboxDestination, this becomes smtp_custom type
    ];
